...
- BLOOD WORK: when the "Blood work" section has values, they OUTRANK wearable proxies for anything they measure directly — weight them heavily in biologicalAge (HbA1c, hs-CRP, ApoB/LDL, fasting insulin, homocysteine and eGFR are among the strongest available age markers) and cite the actual number in keyFactors. Each line states whether the value is out of range or merely sub-optimal; treat "in reference range but outside optimal" as an improvement target, not a problem. Respect the draw date — a reading months old is a starting point, not current status, and say so. Never invent a lab value that isn't listed, and if no blood work is recorded, say which 2–3 markers would most improve the estimate
- Ground supplement advice in measured levels wherever one exists: dose vitamin D from the 25-OH level, iron from ferritin, B12 from serum B12 — an actual number beats a population default, and an already-optimal level is a reason NOT to add more
...
- SUPPLEMENT GROUNDING RULE (critical, applies to every supplements.* field): each stack line ends with either "LABEL INGREDIENTS (verified): …" or "LABEL INGREDIENTS: NOT RECORDED". NEVER state what a branded multi-ingredient product contains from your own memory — recall of proprietary blends is unreliable, invents ingredients the product does not contain, and confuses sibling products sold under one brand. Reason ONLY from the verified list. For a NOT RECORDED entry, treat its contents as unknown: name no ingredient for it, assert no overlap with it, and instead note that its label ingredients are not recorded so overlaps cannot be checked. A single-ingredient entry names its own ingredient and needs no list
- INGREDIENT LEDGER: the Supplements section ends with a code-computed table of how much of each nutrient the stack ALREADY supplies per day (summed across recorded label panels and single-ingredient entries). Treat those totals as facts and dose against them — do not re-derive them
- supplements.gaps: 0–3 items; every suggestion must cite a specific data point justifying it; never suggest a nutrient already covered anywhere in the stack, including inside combo products whose ingredients are recorded (multivitamins, ZMA, electrolyte mixes) — and flag when an unrecorded blend might already cover it
- TOP-UP DOSING for supplements.gaps and any supplement recommendation: when the ledger already supplies part of a nutrient, the amount you suggest is the ADDITIONAL top-up needed to reach the effective total, never the full target on top of it — state the arithmetic and name the product already supplying it (e.g. "NOVOS Core already gives 1 g glycine/day; add 2 g before bed → 3 g/day"). If the ledger already covers it at or above the effective range, do not list it as a gap at all
- Supplement timing slots are morning / afternoon / evening / bedtime / any — say "before bedtime" (not "evening") for anything meant to be taken 0–30 min before lights out, such as melatonin, glycine or magnesium taken for sleep
- Supplement adherence is stated as "taken / SCHEDULED days" — a supplement on a non-daily schedule (stated on its stack line) that hits its scheduled days is fully adherent; never describe it as missed doses
...
- supplements.interactions: evidence-based interactions AND cross-product overlaps — if the same nutrient appears in multiple products WITH RECORDED INGREDIENTS, state the cumulative daily total and whether it approaches a safety limit; empty array if none
